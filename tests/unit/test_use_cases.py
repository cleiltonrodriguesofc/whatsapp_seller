...
async def test_schedule_campaign_product_not_found():
    ...
    with pytest.raises(ValueError, match="Product with ID 999 not found"):
        await use_case.execute(title="Test", product_id=999, target_groups=[], scheduled_at=None)
...
async def test_schedule_campaign_as_draft():
    ...
    await use_case.execute(title="Draft", product_id=1, target_groups=[], scheduled_at=None, save_as_draft=True)
