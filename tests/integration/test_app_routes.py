...
def test_list_products_with_auth(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(email="prod_test@test.com", hashed_password=auth.hash_password("pw123"))
    db_session.add(user)
    db_session.commit()
    
    client.post("/login", data={"username": "prod_test@test.com", "password": "pw123"})
...
def test_new_campaign_form_with_auth(client, db_session):
    auth = AuthService()
    user = UserModel(email="camp_test@test.com", hashed_password=auth.hash_password("pw123"))
    db_session.add(user)
    db_session.commit()
    
    client.post("/login", data={"username": "camp_test@test.com", "password": "pw123"})
...
def test_campaigns_list_with_auth(client, db_session):
    auth = AuthService()
    user = UserModel(email="list_test@test.com", hashed_password=auth.hash_password("pw123"))
    db_session.add(user)
    db_session.commit()
    
    client.post("/login", data={"username": "list_test@test.com", "password": "pw123"})
