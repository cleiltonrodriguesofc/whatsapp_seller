...
def test_status_campaigns_list_auth(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(email="status_list@test.com", hashed_password=auth.hash_password("t3st_p@ssw0rd_2026"))
    db_session.add(user)
    db_session.commit()
    
    # Login
    client.post("/login", data={"username": "status_list@test.com", "password": "t3st_p@ssw0rd_2026"})
...
def test_new_status_campaign_form_auth(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(email="status_form@test.com", hashed_password=auth.hash_password("t3st_p@ssw0rd_2026"))
    db_session.add(user)
    db_session.commit()
    
    client.post("/login", data={"username": "status_form@test.com", "password": "t3st_p@ssw0rd_2026"})
...
def test_create_status_campaign_auth(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(email="status_create@test.com", hashed_password=auth.hash_password("t3st_p@ssw0rd_2026"))
    db_session.add(user)
    db_session.commit()
    ...
    client.post("/login", data={"username": "status_create@test.com", "password": "t3st_p@ssw0rd_2026"})
