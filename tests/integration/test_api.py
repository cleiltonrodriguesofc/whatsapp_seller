...
def test_successful_login(client, db_session):
    # Create user
    auth = AuthService()
    user = UserModel(email="auth_test@test.com", hashed_password=auth.hash_password("mock_auth_val_x9"))
    db_session.add(user)
    db_session.commit()

    response = client.post(
        "/login", data={"username": "auth_test@test.com", "password": "mock_auth_val_x9"}, follow_redirects=True
    )
