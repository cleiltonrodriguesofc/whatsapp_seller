...
def test_request_withdrawal_insufficient_funds(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(email="poor_referral@example.com", hashed_password=auth.hash_password("pass"), referral_balance=50.0)
    db_session.add(user)
    db_session.commit()
...
def test_request_withdrawal_success(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(email="rich_referral@example.com", hashed_password=auth.hash_password("pass"), referral_balance=150.0)
    db_session.add(user)
    db_session.commit()
