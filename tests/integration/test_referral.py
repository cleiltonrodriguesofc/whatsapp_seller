...
def test_referral_dashboard_renders(client, db_session):
    ...
    auth = AuthService()
    user = UserModel(
        email="referral_test@example.com", hashed_password=auth.hash_password("pass")
    )
    db_session.add(user)
    db_session.commit()
    ...
    conversion = ReferralConversionModel(
        referrer_id=user.id, referred_id=2, status="rewarded", reward_brl=29.10
    )
    db_session.add(conversion)
    db_session.commit()

    client.post(
        "/login", data={"username": "referral_test@example.com", "password": "pass"}
    )
...
def test_request_withdrawal_insufficient_funds(client, db_session):
    ...
    client.post(
        "/login", data={"username": "poor_referral@example.com", "password": "pass"}
    )
    response = client.post(
        "/referral/request-withdrawal",
        data={"pix_key": "my_pix_key"},
    ...
    assert (
        "isuficiente" in response.text.lower()
        or "insuficiente" in response.text.lower()
    )
...
def test_request_withdrawal_success(client, db_session):
    ...
    client.post(
        "/login", data={"username": "rich_referral@example.com", "password": "pass"}
    )
    response = client.post(
        "/referral/request-withdrawal",
        data={"pix_key": "my_pix_key"},
