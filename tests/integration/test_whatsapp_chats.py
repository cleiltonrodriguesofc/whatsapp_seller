...
def setup_user_and_instance(db_session, salt):
    auth = AuthService()
    user = UserModel(
        email=f"tester_{salt}@test.com", hashed_password=auth.hash_password("pass")
    )
    db_session.add(user)
    db_session.commit()
...
def test_view_whatsapp_chats_endpoint(client, db_session):
    ...
    client.post(
        "/login", data={"username": "tester_chats@test.com", "password": "pass"}
    )

    with patch(
        "core.presentation.web.routers.whatsapp.EvolutionWhatsAppService.get_active_chats"
    ) as mock_get_chats:
...
def test_get_chat_messages_api(client, db_session):
    ...
    with patch(
        "core.presentation.web.routers.whatsapp.EvolutionWhatsAppService.get_chat_messages"
    ) as mock_get_msgs:
        ...
        res = client.get(
            f"/chats/messages?jid=123@s.whatsapp.net&instance_id={instance.id}"
        )
        assert res.status_code == 200
        data = res.json()
...
def test_sync_whatsapp_targets_prioritizes_remote_jid(client, db_session):
    ...
    with (
        patch(
            "core.presentation.web.routers.whatsapp.EvolutionWhatsAppService.get_groups"
        ) as mock_groups,
        patch(
            "core.presentation.web.routers.whatsapp.EvolutionWhatsAppService.get_contacts"
        ) as mock_contacts,
        patch(
            "core.presentation.web.routers.whatsapp.EvolutionWhatsAppService."
            "get_phonebook_contacts"
        ) as mock_phonebook,
    ):
        # Evolution mock responses
        mock_groups.return_value = [
            {"id": "grp1@g.us", "subject": "Group 1", "name": "Group 1"}
        ]
        mock_contacts.return_value = [
            {
                "id": "internal-uuid",
                "remoteJid": "sync1@s.whatsapp.net",
                "name": "Contact Synced 1",
            }
        ]
