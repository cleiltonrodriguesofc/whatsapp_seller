...
async def referral_dashboard(
    ...
    ref_code_obj = (
        ...
        .filter(ReferralCodeModel.user_id == current_user.id)
        .first()
    )
    ...
    conversions = (
        ...
        .filter(ReferralConversionModel.referrer_id == current_user.id)
        .all()
    )
    ...
    total_converted = len(
        [c for c in conversions if c.status in ("converted", "rewarded")]
    )
...
async def request_withdrawal(
    ...
    if current_user.referral_balance < 100:
        raise HTTPException(
            status_code=400, detail="Saldo insuficiente para saque (mínimo R$ 100,00)."
        )
