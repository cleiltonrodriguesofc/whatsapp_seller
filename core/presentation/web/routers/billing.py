...
async def pricing_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request=request, name="landing.html", context={"plans": plans, "scroll_to": "pricing"})
...
async def create_checkout_session(
    ...
    if not plan or not plan.mp_plan_id:
        raise HTTPException(status_code=404, detail="Plan not found or not configured for Mercado Pago")
...
async def checkout_success(request: Request):
    return templates.TemplateResponse(request=request, name="checkout_status.html", context={"status": "success", "message": "Sua assinatura está sendo processada!"})

@router.get("/checkout/cancel")
async def checkout_cancel(request: Request):
    return templates.TemplateResponse(request=request, name="checkout_status.html", context={"status": "cancel", "message": "O pagamento foi cancelado."})

@router.post("/webhooks/mercadopago")
async def mp_webhook(request: Request, db: Session = Depends(get_db)):
    data = await request.json()
    # We check for preapproval (subscription) updates
    if data.get("action") == "created" or data.get("type") == "subscription_preapproval":
        ...
        subscription = db.query(SubscriptionModel).filter(SubscriptionModel.user_id == user_id).first()
        if subscription:
...
def handle_referral_reward(user_id: int, db: Session):
    ...
    conversion = db.query(ReferralConversionModel).filter(
    ...
    if conversion:
        ...
        if subscription and subscription.plan:
            ...
            referrer = db.query(UserModel).filter(UserModel.id == conversion.referrer_id).first()
            if referrer:
