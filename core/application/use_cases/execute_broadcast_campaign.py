...
class ExecuteBroadcastCampaignUseCase:
    ...
    async def execute(self, campaign_id: int):
        ...
        campaign = self.broadcast_repo.get_by_id(campaign_id, user_id=None)  # user_id=None because scheduler runs as system
        if not campaign or campaign.status in ["sent", "failed"]:
            logger.warning("[broadcast] campaign %s already done or not found, skipping", campaign_id)
            return
        ...
        if campaign.target_type == "list" and campaign.list_id:
            ...
            members = self.list_repo.db.query(BroadcastListMemberModel).filter_by(list_id=campaign.list_id).all()
            for m in members:
            ...
            for jid in campaign.target_jids:
                target_model = self.target_repo.db.query(WhatsAppTargetModel).filter_by(user_id=campaign.user_id, jid=jid).first()
        ...
        if not instance_model:
            logger.error("[broadcast] instance %s not found for campaign %s", campaign.instance_id, campaign_id)
            campaign.status = "failed"
            self.broadcast_repo.save(campaign)
            return

        svc = EvolutionWhatsAppService(instance=instance_model.name, apikey=instance_model.apikey)
        ...
        if campaign.image_url:
            try:
                logger.info("[broadcast] downloading and optimizing image from: %s", campaign.image_url[:80])
                optimized_media = await get_optimized_base64(campaign.image_url, max_size=(1000, 1000), quality=80)
                logger.info("[broadcast] image ready, size: %d chars (base64)", len(optimized_media))
            except Exception as e:
                logger.error("[broadcast] failed to prepare media: %s", e, exc_info=True)
        ...
        for i, target in enumerate(targets):
            ...
            try:
                ...
                if not is_group and jid != "status@broadcast":
                    ...
                    logger.debug("[broadcast] simulating typing for %s (%.1fs)", jid, typing_duration)
                    await svc.set_presence(jid, "composing", delay=int(typing_duration * 1000))
                ...
                logger.error("[broadcast] error sending to %s: %s", jid, e, exc_info=True)
                failed_count += 1
            ...
            if i < len(targets) - 1:
                jitter = random.uniform(15, 60)  # reduced from 30-120s since groups don't need as much gap
